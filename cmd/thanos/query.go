...
func runQuery(
	...
	var (
		storeClients = func() []store.Client {
		...
		rulesClients = func() []rulespb.RulesClient {
			var ret []rulespb.RulesClient
			for _, ss := range storeSets {
				ret = append(ret, ss.GetRulesClients()...)
			}
			return ret
		}
		targetClients = func() []targetspb.TargetsClient {
			var ret []targetspb.TargetsClient
			for _, ss := range storeSets {
				ret = append(ret, ss.GetTargetsClients()...)
			}
			return ret
		}
		metadataClients = func() []metadatapb.MetadataClient {
			var ret []metadatapb.MetadataClient
			for _, ss := range storeSets {
				ret = append(ret, ss.GetMetadataClients()...)
			}
			return ret
		}
		exemplarsStores = func() []*exemplarspb.ExemplarStore {
			var ret []*exemplarspb.ExemplarStore
			for _, ss := range storeSets {
				ret = append(ret, ss.GetExemplarsStores()...)
			}
			return ret
		}

		proxy            = store.NewProxyStore(logger, reg, storeClients, component.Query, selectorLset, storeResponseTimeout)
		rulesProxy       = rules.NewProxy(logger, rulesClients)
		targetsProxy     = targets.NewProxy(logger, targetClients)
		metadataProxy    = metadata.NewProxy(logger, metadataClients)
		exemplarsProxy   = exemplars.NewProxy(logger, exemplarsStores, selectorLset)
