	...
	for w := 0; w < concurrency; w++ {
		go func() {
			...
			for n := 0; n < queriesPerWorker; n++ {
				...
				ps, err := preloadSeries(context.Background(), matchers, seriesLimiter, indexReader)
				testutil.Ok(b, err)
				seriesSet, _, err := blockSeries(context.Background(), nil, indexReader, chunkReader, matchers, chunksLimiter, seriesLimiter, req.SkipChunks, req.MinTime, req.MaxTime, req.Aggregates, ps)
				testutil.Ok(b, err)
